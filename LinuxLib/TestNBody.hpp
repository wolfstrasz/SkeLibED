 ...
 */
	...
	void dnbody2(std::vector<Particle> &particles) {
		...
		auto nbody_init = DynamicMap2(init, threads);
		auto nbody_simulate_step = DynamicMap2(move, threads);
	...
	void dnbody1(std::vector<Particle> &particles) {
		...
		auto nbody_init = DynamicMap1(init, threads);
		auto nbody_simulate_step = DynamicMap1(move, threads);
	...
	void dnbody5(std::vector<Particle> &particles) {
		...
		auto nbody_simulate_step = DynamicMap5(move, threads);
	...
	void test(size_t threadcount, size_t blockcount, size_t np, size_t iters) {
		...
		//	
		//}
		//std::cout << "DMAP6: " << std::to_string(time.count()/nbody_test_count) << std::endl;
